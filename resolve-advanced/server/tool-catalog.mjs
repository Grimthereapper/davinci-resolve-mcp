...
export const CATALOG = [
  ...
  {
    id: 'look_coverage',
    summary: 'Does a look\'s validation SET populate all three tonal bands? Names the band it was never tested in',
    when_to_use: 'Before shipping a look/LUT built on hero frames — the untested band is where a badly-built look breaks',
    inputs: ['pngs', 'minFramesPerBand'],
    gate: 'none',
    not_for: 'judging whether the look is any good (this only checks the evidence covers the range)',
    output: 'covered / untested / thin bands + per-band frame counts + warnings',
    tool: 'drx',
    action: 'look_coverage',
  },
  {
    id: 'scope_read',
    summary: 'Read a frame: parade balance (whole-frame + shadow/mid/highlight bands), vectorscope skin-line, black-balance, %clip/%crush + intent signals',
    when_to_use: 'QC a frame / feed the matchers stats / locate WHICH tonal band carries a cast before correcting it / derive shot-intent signals (low-key, monochromatic, split-toned)',
    inputs: ['png', 'rect'],
    gate: 'none',
